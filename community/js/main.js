...
let boardInfoUrl = `http://${BACKEND_URL}:${BACKEND_PORT}/api/boards?page=${page}&limit=${limit}`;
...
document.querySelector('.pagination').addEventListener('click', async event => {
    try {
        ...
        boardInfoUrl = `http://${BACKEND_URL}:${BACKEND_PORT}/api/boards?page=${page}&limit=${limit}`;
        await loadPage();
    } catch (e) {
        console.error(e);
    }
